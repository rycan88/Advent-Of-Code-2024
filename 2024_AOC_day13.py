...
def parse(string):
    x = int(string[string.index("X") + 2: string.index(",")])
    y = int(string[string.index("Y") + 2:])
...
btokens = 1
for machine in L:
    B = list(map(parse, machine.split("\n")))
    t1, t2, t3 = B
    if (t3[0] % math.gcd(t1[0], t2[0]) != 0 or t3[1] % math.gcd(t1[1], t2[1]) != 0):
        continue
    
    if (t2[0] * atokens - t1[0] * btokens):
        lessA = True
    else:
        lessA = False

    if lessA:
        for a in range(math.ceil(t3[0] / t1[0])):
            if (t3[0] - (t1[0] * a)) % t2[0] == 0:
                b = (t3[0] - (t1[0] * a)) / t2[0]
                if (t1[1] * a + t2[1] * b == t3[1]):
                    total += a * atokens + b * btokens
                    break
    else:
        for y in range(math.ceil(t3[0] / t2[0])):
            if (t3[0] - (t2[0] * b)) % t1[0] == 0:
                y = (t3[0] - (t2[0] * b)) / t1[0]
                if (t2[1] * b + t1[1] * a == t3[1]):
                    total += a * atokens + b * btokens
                    break
print(total)
'''
...
def extendedEuclideanAlgo(x1, x2, x3):
    for a in range(math.ceil(x3 / x1)):
        if (x3 - (x1 * a)) % x2 == 0:
            b = (x3 - (x1 * a)) / x2
            return [a, b]
    return False
...
for machine in L:
    ...
    gcd1 = math.gcd(t1[0], t2[0])
    gcd2 = math.gcd(t1[1], t2[1])

    if (t2[0] * atokens - t1[0] * btokens): # We want less a than b
        tup1 = extendedEuclideanAlgo(t1[0], t2[0], t3[0])
        tup2 = extendedEuclideanAlgo(t1[1], t2[1], t3[1])
        if (tup1 == False or tup2 == False):
            continue 
        ...
        #(tup1[0] + (t2[0] / gcd1) * x) * t1[0] + (tup1[1] - (t1[0] / gcd1) * x) * t2[0] == t3[0]
        #(tup2[0] + (t2[1] / gcd2) * y) * t1[1] + (tup2[1] - (t1[1] / gcd2) * y) * t2[1] == t3[1]

        #(tup1[0] + (t2[0] / gcd1) * x) == (tup2[0] + (t2[1] / gcd2) * y)
        #(tup1[1] - (t1[0] / gcd1) * x) == (tup2[1] - (t1[1] / gcd2) * y)

        #(tup1[0] + d * x) == (tup2[0] + f * y)
        #(tup1[1] - c * x) == (tup2[1] - e * y)

        #y = (tup1[0] - tup2[0] + d * x) / f 
        #(tup1[1] - c * x) == (tup2[1] - e * y)

        # (tup1[1] - c * x) == (tup2[1] - e * (tup1[0] - tup2[0] + d * x) / f )
        # ((tup1[1] - tup2[1]) * f ) ==  (-e * tup1[0] + e * tup2[0] + (c + f- e * d )* x)

        x = ((tup1[1] - tup2[1]) * f + e * (tup1[0] - tup2[0])) / (c * f - e * d)
